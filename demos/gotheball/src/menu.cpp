// Time-stamp: <2009-04-05 22:05:53 geg>
//
...
Action Entry::update(vector<bool>& keys, Mouse& mouse,
                     unsigned int delta)
{
        Action action;
        Sprite * s;
...
void Score_menu::init ()
{
        ...
        int x, y=84, count=0;
        Sprite * font;
        ifstream in (HOMEDIR "/.gotheball-scores");
...
void Score_menu::send_data (map<string,string>  data)
{
        ...
        f.open(HOMEDIR "/.gotheball-scores");
...
Underscore::Underscore (int x, int y)
        : Sprite(DATADIR "/sprites/underscore.png",false,255,255,255)
{
